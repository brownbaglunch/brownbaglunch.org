---
layout: talk
url: speakers/delphine-raymond/talks/boite-a-outils-du-product-owner-product-manager/
tags:
- product-owner
- product-manager
- agile
versions:
- lang: FR
  title: Boîte à outils du Product Owner / Product Manager
  abstract: Vous souhaitez découvrir la boîte à outils du PO / PM pour construire
    la vision de votre produit, sa roadmap, identifier vos personas ou les jobs-to-be-done,
    définir le MVP de votre produit, créer et prioriser un backlog de fonctionnalités,
    définir une story map, rédiger une user story ou une job story, les estimer...
---

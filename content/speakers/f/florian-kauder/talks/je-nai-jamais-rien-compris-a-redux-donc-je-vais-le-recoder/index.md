---
layout: talk
url: speakers/florian-kauder/talks/je-nai-jamais-rien-compris-a-redux-donc-je-vais-le-recoder/
tags:
- redux
- react
- javascript
- architecture
- patterns
- coding-live
- frontend
versions:
- lang: FR
  title: Je n'ai jamais rien compris à Redux, donc je vais le recoder
  abstract: 'Ola Voyageur·e !


    Si le temps vous le permet, je m''en vais vous conter les aventures du sieur Abramov
    et de sa monture Redux.

    En ayant cotoyé l''écurie React, votre seule rencontre avec cet équidé ne put
    être qu''un dédale de mésaventures.

    Mais soit, votre serviteur à l''esprit perverti s''est entêté à découvrir le secret
    de ces engrenages.

    Par la création, je m''en vais allumer la lanterne de vos pensées.

    Mensonge, baliverne, tromperie, magie noire,

    Rencontrez moi dans la salle du village,

    Et découvrez mes humbles secrets.'
---

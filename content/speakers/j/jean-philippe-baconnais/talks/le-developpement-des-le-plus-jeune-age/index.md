---
layout: talk
url: /speakers/jean-philippe-baconnais/talks/le-developpement-des-le-plus-jeune-age/
tags:
- dev
- kids
versions:
- label: FR
  flag: fr
  title: Le développement dès le plus jeune âge
  abstract: Depuis ma sortie d'école, je fais du développement. Mais il existe beaucoup
    d'actions et d'évènements autour du développement pour les enfants notamment via
    le Devoxx4Kids. Je vous raconterai comment s'est déroulée une session à laquelle
    j'ai participé et vous découvrirez comment vos enfants pourront vous impressionner
    avec leurs premiers développements !
---

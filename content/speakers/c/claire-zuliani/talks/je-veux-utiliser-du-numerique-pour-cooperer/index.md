---
layout: talk
url: speakers/claire-zuliani/talks/je-veux-utiliser-du-numerique-pour-cooperer/
tags:
- outils
- cooperation
- strategie
versions:
- label: FR
  flag: fr
  title: Je veux utiliser du numérique pour coopérer
  abstract: Vous avez un projet impliquant plein d'acteurs différents, vous vous dites
    que le numérique pourrait vous filer un coup de main mais ne savez pas par quel
    bout le prendre. Discutons ensemble de vos usages, des outils existants et de
    comment le numérique s'insère dans une stratégie plus globale de coopération.
---

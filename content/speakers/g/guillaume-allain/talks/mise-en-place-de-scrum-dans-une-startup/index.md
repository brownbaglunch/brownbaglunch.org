---
layout: talk
url: /speakers/guillaume-allain/talks/mise-en-place-de-scrum-dans-une-startup/
tags:
- scrum
- startup
- retour-d-experience
versions:
- label: FR
  flag: fr
  title: Mise en place de Scrum dans une startup
  abstract: On parle souvent de Scrum pour rendre plus agile des procédures rigides.
    J'ai mis scrum en place chez Lucca pour scruturer notre équipe de R&D qui n'avait
    aucune procédure. Si votre équipe de R&D augmente et que vous avez besoin de vous
    structurer pour rester efficace. Je vous partagerai les étapes que l'on a utilisées
    chez Lucca pour mettre en place Scrum.
---

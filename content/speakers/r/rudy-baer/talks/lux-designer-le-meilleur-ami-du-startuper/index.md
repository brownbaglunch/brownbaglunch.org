---
layout: talk
url: /speakers/rudy-baer/talks/lux-designer-le-meilleur-ami-du-startuper/
tags:
- ui
- ux
- startup
- startuper
- design
- ergonomie
versions:
- label: FR
  flag: fr
  title: L'UX Designer, le meilleur ami du startuper !
  abstract: ' "L''UX Design" on en entend souvent parler sans forcément savoir ce
    que c''est ou en réduisant ça à l''aspect graphique... Après avoir défini quelques
    notions théoriques et les points importants à avoir en tête, je vous expliquerai
    dans cette conférence pourquoi l''UX Designer est un atout pour les CTO, les développeurs,
    le marketing, vos utilisateurs et donc pour votre business !'
---
